# CodeBuddy Multi-Model Secure Bridge / CodeBuddy 多模型安全接入

Bridge your own **OpenAI Codex / ChatGPT** and **Google Antigravity / Gemini** subscriptions into **CodeBuddy CN** through a loopback-only CLIProxyAPI proxy listening on `127.0.0.1:8317`.
...
> **EN** This skill is a CodeBuddy adaptation of the [WorkBuddy multi-model secure bridge](https://github.com/router-for-me). Key differences:
...
>
> **中文** 本技能是 [WorkBuddy 多模型安全接入](https://github.com/router-for-me) 的 CodeBuddy 适配版。主要差异：
...
## Install into CodeBuddy / 安装到 CodeBuddy
...
```bash
python3 <skill-dir>/scripts/bridge.py audit      # audit first, write nothing / 先审计，不写配置
python3 <skill-dir>/scripts/bridge.py bootstrap   # preview the local bridge / 预览本地桥接
python3 <skill-dir>/scripts/bridge.py sync        # sync catalog models into models.json / 把目录里存在的模型同步进 models.json
...
python3 <skill-dir>/scripts/bridge.py sync --skip-probes   # skip probes entirely, register all catalog models (recommended when quota is exhausted) / 完全跳过探测，直接按清单注册全部目录模型（配额耗尽时推荐）
python3 <skill-dir>/scripts/bridge.py sync --strict        # restore original strict behavior: drop a model if its probe fails / 恢复原始严格行为：探测不过就跳过该模型
python3 <skill-dir>/scripts/bridge.py validate-provider providers/codex.json
```

> **EN** If `codebuddy_stale_system_proxy` appears, a leftover proxy was detected on 8080/8081. Fully quit and reopen CodeBuddy before retrying — do **not** change the system proxy or built-in models to work around it.
>
> **中文** 若出现 `codebuddy_stale_system_proxy`，说明检测到 8080/8081 上有遗留代理。请完全退出并重新打开 CodeBuddy 后再重试，**不要**去改系统代理或内置模型来绕过。
...
## Security Model (Key Points) / 安全模型（要点）
...
## Install Source & Trust Level / 安装来源与信任等级
...
## Provider Manifest Maintenance / Provider 清单维护
